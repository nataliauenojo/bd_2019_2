...
-- SELECT * FROM ALUNO A WHERE A.idade<20 AND A.nivel='JR';
 
-- Selecione os professores que são do departamento  68
-- SELECT * FROM PROFESSOR P WHERE P.idDepto=68;
 
-- Selecione os alunos que se matricularam em curso de 'Database Systems'
-- SELECT * FROM ALUNO A,MATRICULADO M , CURSO C WHERE A.nroAlun=M.nroAlun AND C.nome=M.nomeCurso AND M.nomeCurso='Database Systems';
 
-- Selecione os nomes dos professores que ministraram algum curso na sala '20 AVW'
-- SELECT P.nomeProf FROM PROFESSOR P,CURSO C WHERE P.idProf=C.idProf AND C.sala='20 AVW';
